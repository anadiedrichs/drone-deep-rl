...
class DroneOpenAIGymEnvironment(Supervisor, gym.Env):
    ...
    def __init__(self, max_episode_steps=1000):
        ...
        self.observation_space = Box(low=np.array([- pi, -pi/2, - pi, 0,0, 0.1, 2.0, 2.0, 2.0, 2.0,0]),
                                     high=np.array([pi, pi/2, pi, 10, 10, 5,2000, 2000, 2000, 2000,6]),
                                     dtype=np.float64)
    ...
    def initialization(self):
        ...
        self.human = Copilot("forward")
        self.copilot_action = 0
    ...
    def get_observations(self):
        ...
        # Copilot shared autonomy input
        self.copilot_action = self.human.go_forward_pilot()
        
        # Normalize values for RL model 
        copilot_action = normalize_to_range(self.copilot_action,0, 6, -1.0, 1.0,clip=True)
        ...
        arr = [roll, pitch, yaw_rate, v_x, v_y, self.altitude ,range_front_value, range_back_value ,range_right_value, range_left_value, copilot_action ]
    ...
    def get_reward(self, action=6):
        ...
        # follow the pilot         
        if action == self.copilot_action :
            r_copilot = 1
        else:
            r_copilot = -1
        
        # be near an obstacle
        too_close = (self.dist_front <= 200 or self.dist_left <= 200 or \
               self.dist_right <= 200 or self.dist_back <= 200 )
        ...
        #TODO add w1 & w2
        reward =  r_copilot +  r_avoid_obstacle
    ...
    #TODO add to stop when cummulative reward reach some level
    def is_done(self):
        """
        ...
        if bool(self.dist_front <= 100 or self.dist_left <= 100 or \
               self.dist_right <= 100 or \
               self.dist_back <= 100 ) :
            return True
...
class Copilot:
    ...
    def __init__(self, name):
        ...
        # 4 turn left
        # 5 turn right
        # 6 keep on the same place
        self.actions = [0,1,2,3,4,5,6]
        
    # always go forward copilot 
    def go_forward_pilot(self):
        return 0
        
    # Random or noisy copilot
    def random_pilot(self):
      indice_aleatorio = random.randrange(len(self.actions))
      return self.actions[indice_aleatorio]
